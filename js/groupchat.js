        ...
        success:function(response){
           $('#ajaxload').val(0);
            ...
            $.each(response, function(key,value){
                $('#idadminame').html(value.chat_title);
				$('#idgroupname').html(value.chat_room_name);
                ...
                {
                     var likelink = "<span> Support </span><span>&nbsp;"+nooflikes+"  Support this<span>";
                }
                else
                {
                     var likelink = "<a href = 'javascript:void(0)' class = 'likemsg' data-msgid = '"+value.message_id+"' > Support</a><span>&nbsp;"+nooflikes+"  Support this<span>";
                }
               if (value.file_type == 0) // if file_type = 0, then that means it is a text message
                {
                    messages = messages+"<li class='media'><div class='media-body'><div class='media'><a class='pull-left' href='#'><img class='media-object img-circle ' height = '75px' width = '75px' src='"+proimgfold+"/"+value.profile_image+"' /></a><div class='media-body' >"+value.message+"  <br /> <small class='text-muted'>"+value.first_name+" "+value.last_name+" | "+value.date_time+likelink+"</small><hr /></div></div></div></li>";
                }
                else if (value.file_type == 1)
                {
                    messages = messages+"<li class='media'><div class='media-body'><div class='media'><a class='pull-left' href='#'><img class='media-object img-circle ' height = '75px' width = '75px' src='"+proimgfold+"/"+value.profile_image+"' /></a><div class='media-body' ><img width = '100px' height = '100px' src = '"+chatimgfold+"/"+value.file_name+"'  /><br /> <small class='text-muted'>"+value.first_name+" "+value.last_name+" | "+value.date_time+likelink+"</small><hr /></div></div></div></li>";
                }
            })
