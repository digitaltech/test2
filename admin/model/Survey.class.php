...
class Survey
{
    ...
    {
        //die(print_r($params));
         $qrystmt = $this->dbconn->prepare("INSERT INTO ".$this->tb_prefix."survey (topic,constituency_id)  VALUES (:topic,(SELECT constituency_id from ".$this->tb_prefix."users WHERE user_id = :user_id)) ");
         $arrVals = array('topic'=>$params['topic'],'user_id'=>$_SESSION['user_id']);
         $qrystmt->execute($arrVals) or die(print_r($qrystmt->errorInfo(),TRUE));
         
         $qrystmt = $this->dbconn->prepare("UPDATE ".$this->tb_prefix."users SET noti_survey = :noti_survey WHERE constituency_id = :constituency_id");
         $arrVals = array('noti_survey'=>1,'constituency_id'=>$_SESSION['constituency_id']);
         $qrystmt->execute($arrVals) or die(print_r($qrystmt->errorInfo(),TRUE));
         return TRUE;
    }
    ...
    function getAll()
    {
        $qrstmt = $this->dbconn->prepare("SELECT * FROM ".$this->tb_prefix."survey");
        $arrVals = array();
        $qrstmt->execute($arrVals) or die(print_r($qrstmt->errorInfo(),TRUE));
        $result = array();
        $row  = array();
    ...
    {
         $qrystmt = $this->dbconn->prepare("UPDATE ".$this->tb_prefix."survey SET topic = :topic WHERE serial_no = :serial_no ");
