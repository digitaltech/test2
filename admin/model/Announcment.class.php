...
class Announcment
{
    ...
    function add($params,$file,$imageFileType)
    {
        ...
        $qrystmt->execute($arrVals) or die(print_r($qrystmt->errorInfo(),TRUE));
         ...
         $qrystmt = $this->dbconn->prepare("UPDATE ".$this->tb_prefix."users SET noti_post = :noti_post WHERE constituency_id = :constituency_id");
         $arrVals = array('noti_post'=>1,'constituency_id'=>$_SESSION['constituency_id']);
         $qrystmt->execute($arrVals) or die(print_r($qrystmt->errorInfo(),TRUE));
    ...
    function getAll()
    {
        $qrystmt = $this->dbconn->prepare("SELECT announcment.* FROM ".$this->tb_prefix."announcment as announcment INNER JOIN ".$this->tb_prefix."constituency as constituency ON announcment.constituency_id = constituency.serial_no ");
        $arrVals = array();
        $qrystmt->execute($arrVals) or die(print_r($qrystmt->errorInfo(),TRUE));
        $result  = array();
        $row   = array();
    ...
    function update($params,$file,$imageFileType)
    {
		//die(print_r($params));
